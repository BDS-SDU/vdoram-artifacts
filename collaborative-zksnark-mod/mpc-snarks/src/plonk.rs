...
pub fn local_test_prove_and_verify(n_iters: usize) {
    ...
    let compute_timer = start_timer!(|| "Squaring-Compute");    
    ...
    let setup_timer = start_timer!(|| "Squaring-Setup");
    ...
    let prove_timer = start_timer!(|| "Squaring-Prove");
    let pf = LocalPlonk::prove(&pk, &circ, zk_rng);
    end_timer!(prove_timer);

    let verify_timer = start_timer!(|| "Squaring-Verify");
