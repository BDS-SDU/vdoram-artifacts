...
ssh "$ssh_user_host" -- "cd ~/$PROJECT_REMOTE_DIR_NAME/$MPC_NODE_NUM/bin/ && tmux new-session -d 'tmux setw remain-on-exit on; for repeat_index in "'$(seq 1 '"$EXP_REPEAT_COUNT"'); do INSTANCE_NAME=exp1_single.repeat$repeat_index ./HuayiQi.CollaborativeZkVmExperiment exp-1-run-single; done; echo ==== Experiment 1 done ===='"; source exp-constants.sh && for ins in "'"${EXP23_INSTANCES[@]}"; do echo ==== $ins ==== && for repeat_index in $(seq 1 '"$EXP_REPEAT_COUNT"'); do INSTANCE_NAME=exp23_single."$ins".repeat$repeat_index ./HuayiQi.CollaborativeZkVm run-single-zkvm --output-folder . --program-instance $ins'".instance.0.json; done; done; echo ==== All done ===='"
