...
ssh "$ssh_user_host" -- "cd ~/$PROJECT_REMOTE_DIR_NAME/$MPC_NODE_NUM/bin/ && tmux new-session -d 'tmux setw remain-on-exit on; for repeat_index in "'$(seq 1 '"$EXP_REPEAT_COUNT"'); do INSTANCE_NAME=exp1_mpc_thread.repeat$repeat_index ./HuayiQi.CollaborativeZkVmExperiment exp-1-run-mpc-thread --unsafe-repeat-preshared; sleep 10; done; echo ==== Experiment 1 done ===='"; source exp-constants.sh && for ins in "'"${EXP23_INSTANCES[@]}"; do echo ==== $ins ==== && for repeat_index in $(seq 1 '"$EXP_REPEAT_COUNT"'); do INSTANCE_NAME=exp23_mpc_thread."$ins".repeat$repeat_index ./HuayiQi.CollaborativeZkVmExperiment run-mpc-zkvm-thread --program-instance-name $ins'" --unsafe-repeat-preshared; sleep 10; done; done; echo ==== All done ===='"
