...
ssh "$ssh_user_host" -- "cd ~/$PROJECT_REMOTE_DIR_NAME/$MPC_NODE_NUM/bin/ && tmux new-session -d 'tmux setw remain-on-exit on; for repeat_index in "'$(seq 1 '"$EXP_REPEAT_COUNT"'); do INSTANCE_NAME=exp1_single.repeat$repeat_index ./HuayiQi.CollaborativeZkVmExperiment exp-1-run-single; done; echo ==== All done ===='"'"
