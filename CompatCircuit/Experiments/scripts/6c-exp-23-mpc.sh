...
for ((i = 0; i < $CONTROL_NODE_NUM; i++)); do
    (
        ...
        ssh "$ssh_user_host" -- "cd ~/$PROJECT_REMOTE_DIR_NAME/$MPC_NODE_NUM/bin/ && tmux new-session -d 'tmux setw remain-on-exit on; source exp-constants.sh && for ins in "'"${EXP23_INSTANCES[@]}"; do echo ==== $ins ==== && for repeat_index in $(seq 1 '"$EXP_REPEAT_COUNT"'); do INSTANCE_NAME=exp23_mpc."$ins".repeat$repeat_index ./HuayiQi.CollaborativeZkVmExperiment run-mpc-zkvm --program-instance $ins'".instance.$i.json --party $i --unsafe-repeat-preshared; sleep 10; done; done; echo ==== All done ===='"
    ) &
